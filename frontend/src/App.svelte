<script lang="ts">
  ...
  function blankProgress(): ScanProgress {
    return { filesSeen: 0, filesHashed: 0, bytesHashed: 0, skipped: 0, currentPath: "", done: false };
  }
...
<div class="flex h-full flex-col">
  ...
  <header class="flex items-center gap-3 border-b px-6 py-3">
    ...
    <span class="rounded bg-secondary px-1.5 py-0.5 text-[10px] font-medium uppercase text-muted-foreground">
      read-only
    </span>
  ...
  <section class="border-b px-6 py-4">
    <div class="flex flex-wrap items-end gap-3">
      <div class="flex min-w-0 flex-1 flex-col gap-1">
        <label for="root" class="text-xs font-medium text-muted-foreground">Folder to scan</label>
        <div class="flex gap-2">
          <input
            ...
            spellcheck="false"
            class="h-9 min-w-0 flex-1 rounded-md border border-input bg-background px-3 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
          />
      ...
      <div class="flex flex-col gap-1">
        <label for="algo" class="text-xs font-medium text-muted-foreground">Algorithm</label>
        <div class="relative">
          <select
            id="algo"
            bind:value={algo}
          ...
          >
            <option class="bg-background text-foreground" value="blake3">BLAKE3</option>
            <option class="bg-background text-foreground" value="sha256">SHA-256</option>
          </select>
          <ChevronDown
      ...
      <div class="flex items-center gap-4 pb-2">
        <label class="flex items-center gap-2 text-sm">
          <Switch label="Ignore hidden" checked={ignoreHidden} onCheckedChange={(v) => (ignoreHidden = v)} />
          Ignore hidden
        </label>
        <label class="flex items-center gap-2 text-sm">
          <Switch label="Follow symlinks" checked={followSymlinks} onCheckedChange={(v) => (followSymlinks = v)} />
          Follow symlinks
        </label>
      </div>
    ...
    {#if !wailsReady}
      <p class="mt-3 flex items-center gap-2 text-xs text-amber-500">
        ...
        Desktop bindings not detected — run via <code class="rounded bg-secondary px-1">wails dev</code> or a built binary.
      </p>
    {/if}
  </section>
  ...
  {#if status === "scanning" && progress}
    <section class="border-b px-6 py-3">
      <div class="mb-2 flex items-center gap-2 text-sm">
        ...
        <span class="font-medium">{pluralize(progress.filesHashed, "file")} hashed</span>
        <span class="text-muted-foreground">
          · {pluralize(progress.filesSeen, "file")} seen · {formatBytes(progress.bytesHashed)} read
          {#if progress.skipped > 0}· {progress.skipped} skipped{/if}
        </span>
      </div>
      <Progress indeterminate={true} />
      {#if progress.currentPath}
        <p class="mt-2 truncate text-xs text-muted-foreground">{progress.currentPath}</p>
      {/if}
    </section>
  {/if}
  ...
  <main class="scrollbar-thin flex-1 overflow-y-auto px-6 py-5">
    {#if status === "error"}
      <div class="mx-auto mt-12 max-w-md rounded-lg border border-destructive/40 bg-destructive/10 p-4 text-center">
    ...
    {:else if status === "done" && report}
      {#if report.folderGroups.length === 0 && report.fileGroups.length === 0}
        <div class="mt-16 text-center text-muted-foreground">
          ...
          <p class="text-sm">Nothing in this folder matched on size and content header.</p>
        </div>
      {:else}
        <Results {report} />
      {/if}
    {:else if status === "idle"}
      <div class="mt-16 text-center text-muted-foreground">
        ...
        <p class="mt-2 font-medium text-foreground">Choose a folder and start a scan</p>
        <p class="text-sm">hashdupes finds duplicate files and folders without modifying anything.</p>
      </div>
    {/if}
  </main>
</div>
